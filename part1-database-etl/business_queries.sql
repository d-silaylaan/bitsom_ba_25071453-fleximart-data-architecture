...
-- who have placed at least 2 orders and spent more than ₹5,000. Order by total amount spent in descending order 
-- Expected to return customers with 2+ orders and >5000 spent

SELECT 
c.first_name, c.last_name,c.email,COUNT(DISTINCT o.order_id) AS total_orders,SUM(oi.subtotal) AS total_spent
FROM customers c
JOIN orders o 
...
SELECT p.category,COUNT(DISTINCT p.product_id) AS num_products,
SUM(oi.quantity) AS total_quantity_sold,SUM(oi.subtotal) AS total_revenue
FROM products p
JOIN order_items oi
...
-- total number of orders, total revenue, and the running total of revenue (cumulative revenue from 
