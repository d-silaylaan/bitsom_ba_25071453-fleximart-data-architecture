...
SELECT d.year,d.quarter,d.month_name,SUM(f.total_amount) AS total_sales,SUM(f.quantity_sold) AS total_quantity
FROM fact_sales f
JOIN dim_date d
...
-- Show the top 10 products by revenue, along with their category, total units sold, and 
-- revenue contribution percentage."
-- Includes: Revenue percentage calculation



SELECT p.product_name,p.category,SUM(f.quantity_sold) AS units_sold,SUM(f.total_amount) AS revenue,
ROUND(
(SUM(f.total_amount)*100) / (SELECT SUM(f.total_amount) FROM fact_sales f), 2) AS revenue_percentage
FROM fact_sales f
JOIN dim_product p 
...
-- into 'High Value' (>₹50,000 spent), 'Medium Value' (₹20,000-₹50,000), and 'Low Value' (<₹20,000). 
